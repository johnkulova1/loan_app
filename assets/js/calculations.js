...
function payment(rate_per_period, number_of_payments, present_value,
		future_value, type) {
	if (rate_per_period != 0.0) {
		//alert(rate_per_period+":"+number_of_payments+":"+ present_value+":"+ future_value+":"+ type);
		// Interest rate exists
		var q = Math.pow(1 + rate_per_period, number_of_payments);
		return -((rate_per_period * (future_value + (q * present_value))) / ((-1 + q) * (1 + rate_per_period
				* (type))));
...
function generatePlan(element) {
	...
	if (rate != 0 && nPayments != 0/* && penalty != 0*/ && pv != 0 && e.length != 0) {
		...
		tdMPDate = new Date(), /*=tdActualAmt =tdPenalty =tdAmortization= "",*/
		...
		for (counter = 1; counter <= nPayments; counter++) {
			...
			= tdPrinciple_Inp = tdCB_Inp =
			tdMPDate_Inp = "<textarea class='disabled' rows='1' cols='9' wrap='hard' readonly='readonly' id=";
			tdMonthId = counter;
			//set month and year
			tdMonth = Object.keys(enumMONTHS)[tdMonthCount];
			tdYear = startYear;//enumYEARS[tdYearCount];
			tdMonthCount++;
			if (tdMonthCount > 11) {//iterate over the months and years
				tdMonthCount = 0;
				startYear++;//tdYearCount++;
			}
			//set the MPDate - expected pay date per month
			tdMPDate = tdYearCount+'-'+('0'+(tdMonthCount==0?12:tdMonthCount)).slice(-2)+'-'+startDate;
			//tdMPDate = new Date(tdMPDate);
			/*alert(new Date(tdMPDate*1000).getDate());*/
			//new Date(tdYearCount,tdMonthCount,startDate); 
			if (counter == nPayments) {
				...
				//pass nPayments to the backend using this input
				$('input#inpPayCount').val(nPayments);
			}
